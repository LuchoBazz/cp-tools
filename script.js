...
let getTestCaseFromProblemHtml = (dir, html) => {
  ...
  $('div.input pre').each((i, elem) => {
    ...
    data[i] = {
      ...data[i],
      input: $(elem).html().replace(/<br>/g, "\n")
      //input: $(elem).innerText || elem.textContent
    };
  });
  ...
  console.log(data);
  data.forEach((test, i) => {
    fs.writeFile(`${dir}/testcase_0${i+1}.in`, test.input, function(err) {
      if(err) {
          console.log(err);
      }
      console.log(`The file ${dir}/testcase_0${i+1}.in was saved!`);
    }); 
    fs.writeFile(`${dir}/testcase_0${i+1}.out`, test.output, function(err) {
      if(err) {
          console.log(err);
      }
      console.log(`The file ${dir}/testcase_0${i+1}.out was saved!`);
    }); 
  })
  console.log(data);
}
